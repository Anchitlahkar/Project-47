...
function setup() {
  ...
  borderRight = createSprite(1000, height / 2, 5, height);

  settingBox = createSprite(500, 300, 225, 50);
  settingBox.addImage(SettingButtonImg);
  settingBox.scale = 0.7;

  playBox = createSprite(500, 220, 225, 50);
  playBox.addImage(playButtonImg);
  playBox.scale = 0.5;

  backButton = createSprite(500, 450, 225, 50);
...
function draw() {
  ...
  if (gameState !== "end") drawSprites();

  if (gameState !== "win") drawSprites();
